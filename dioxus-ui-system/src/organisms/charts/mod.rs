...
pub use common::{
    calculate_area_path, calculate_nice_ticks, calculate_smooth_line, color_to_css,
    format_compact_number, format_currency, format_percentage, AnimationEasing, ChartAnimation,
    ChartAxis, ChartDataPoint, ChartMargin, ChartSeries, ChartTooltip, LegendPosition,
};

// Re-export bar chart
pub use bar_chart::{BarChart, BarChartProps, BarChartVariant};

// Re-export line chart
pub use line_chart::{LineChart, LineChartProps, LineChartVariant};

// Re-export pie chart
pub use pie_chart::{DonutChart, GaugeChart, PieChart, PieChartProps, PieChartVariant};

// Re-export sparkline
pub use sparkline::{Sparkline, SparklineProps, SparklineVariant, TrendIndicator};
...
pub mod utils {
    ...
    pub fn normalize_to_percent(data: &[f64]) -> Vec<f64> {
        ...
        data.iter().map(|&v| ((v - min) / range) * 100.0).collect()
    }
}
