...
impl PartialEq for DateRangePreset {
    fn eq(&self, other: &Self) -> bool {
        // Only compare labels, not function pointers
        // Function pointer comparisons are unreliable
                ...
                if let Some(ref start) = start_date {
                    if let (Some(s), Some(d)) = (parse_date(start), parse_date(&date)) {
                        if d >= s {
                            ...
                            if let Some(ref handler) = on_range_change {
                                handler
                                    .call((Some(start_date.clone().unwrap()), Some(start.clone())));
                            }
                        }
                    }
                } else {
...
/// Footer component
#[derive(Props, Clone, PartialEq)]
pub(crate) struct DateRangeFooterProps {
    pub today: Date,
    pub on_select: EventHandler<String>,
    pub on_close: EventHandler<()>,
}
...
fn DateRangeFooter(props: DateRangeFooterProps) -> Element {
    ...
    let today_str = format!(
        "{:04}-{:02}-{:02}",
        props.today.year, props.today.month, props.today.day
    );
...
fn format_date_range(start: &Option<String>, end: &Option<String>) -> String {
    match (
        start.as_ref().and_then(|s| parse_date(s)),
        end.as_ref().and_then(|e| parse_date(e)),
    ) {
        (Some(s), Some(e)) => {
            if s.year == e.year {
