...
pub fn Tour(props: TourProps) -> Element {
    ...
    let (bg_color, fg_color, muted_color, primary_color, border_color, radius_md, shadow_xl) =
        theme_colors();
    ...
    let step = props
        ...
        .get(current_step)
        .cloned()
        .unwrap_or_else(|| props.steps.first().cloned().unwrap());

    let mask_color = props
        .mask_color
        .clone()
        .unwrap_or_else(|| Color::new_rgba(0, 0, 0, 0.5).to_rgba());

    let overlay_style = use_style(|_t| Style::new().fixed().inset("0").z_index(9998).build());
...
mod tests {
    ...
    fn test_tour_step_with_placement() {
        let step =
            TourStep::new("#sidebar", "Nav", "Navigation menu").with_placement(Placement::Right);
